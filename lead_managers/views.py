...
def new_lead_form_view(request):
    if request.method == 'GET':
        ...
        return render(request, 'new_lead_form.html', {'lead_sources': lead_sources})
    ...
    if request.method == 'POST':
        ...
        source = LeadSource.objects.filter(name=data.get('source')).first()
        if data['lead_type'] == TENANT_LEAD:
            ...
            lead = TenantLead.objects.create(name=name, gender=gender, phone_no=phone_no, email=email, source=source,
                                             ...
                                             created_by=lead_manager, managed_by=lead_manager)
            ...
            lead = HouseOwnerLead.objects.create(name=name, gender=gender, phone_no=phone_no, email=email,
                                                 source=source, house_type=house_type, furnish_type=furnish_type,
