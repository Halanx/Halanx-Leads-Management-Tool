...
from affiliate_lead.utils import TENANT_LEAD_REFERRAL_UPDATE_URL, HOUSE_OWNER_LEAD_REFERRAL_UPDATE_URL
from leads.api.serializers import TenantLeadActivitySerializer, HouseOwnerLeadActivitySerializer
from leads.utils import DATA, TASK_TYPE, UPDATE_TENANT_LEAD_ACTIVITY_STATUS, STATUS_DISQUALIFIED, CANCELLED, PENDING, \
    STATUS_CONVERTED, STATUS_VISIT_SCHEDULED, METADATA, UPDATE_HOUSE_OWNER_LEAD_ACTIVITY_STATUS
...
def update_owner_lead_activity_status_in_affiliate_tool(owner_lead_activity):
    request_data = {
        TASK_TYPE: UPDATE_HOUSE_OWNER_LEAD_ACTIVITY_STATUS,
        DATA: HouseOwnerLeadActivitySerializer(owner_lead_activity).data,
        METADATA: {'referral_status': get_appropriate_status_of_lead_activity_for_affiliate_tool(
            lead_activity=owner_lead_activity)}
    }

    owner_referral_id = None
    if owner_lead_activity.lead.referral_id:
        owner_referral_id = owner_lead_activity.lead.referral_id

    if owner_referral_id:
        req = requests.patch(HOUSE_OWNER_LEAD_REFERRAL_UPDATE_URL.format(**{'pk': owner_referral_id}),
                             data=json.dumps(request_data),
                             headers={'Content-type': 'application/json'},
                             timeout=10,
                             auth=(config('AFFILIATE_TOOL_ADMIN_USERNAME'), config('AFFILIATE_TOOL_ADMIN_PASSWORD')))
        ...
        if req.status_code == 200:
            if req.json()[STATUS] == SUCCESS:
                owner_lead_activity.acknowledged_by_affiliate = True
                owner_lead_activity.save()
